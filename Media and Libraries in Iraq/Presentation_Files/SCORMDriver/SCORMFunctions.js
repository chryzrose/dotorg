...
// custom bool variable flags for PointeCast specific calls
var blnScorm12_slideBeacon = false;
var blnScorm12_questionText = false;
var blnScorm12_allowInteractionRetake = false;
...
function SCORM_Initialize(){
    ...
    // initialize custom PointeCast specific API flags
    SCORM_InitializeCustomAPIFlags();
...
//---------------------------------------------------------------------------------
// PointeCast custom methods
function SCORM_InitializeCustomAPIFlags(){
    
    blnScorm12_slideBeacon = false;
    blnScorm12_questionText = false;
    blnScorm12_allowInteractionRetake = false;
    
    WriteToDebug("In SCORM_InitializeCustomAPIFlags");
    SCORM_ClearErrorInfo();
    
    var strCoreChildren = SCORM_CallLMSGetValue("cmi.core._children");
    
    // special check to see if special LMS features exist
    if (strCoreChildren.indexOf("pc_slidebeacon") != -1){
        blnScorm12_slideBeacon = true;
    }
    
    if (strCoreChildren.indexOf("pc_questiontext") != -1){
        blnScorm12_questionText = true;
        blnScorm12_allowInteractionRetake = true;
    }
    
    WriteToDebug("blnScorm12_slideBeacon=" + blnScorm12_slideBeacon + ", blnScorm12_questionText=" + blnScorm12_questionText + ", blnScorm12_allowInteractionRetake=" + blnScorm12_allowInteractionRetake)
}
function SCORM_SetSlideStarted(intSlideID){
    if (blnScorm12_slideBeacon){
        WriteToDebug("In SCORM_SetSlideStarted intSlideID=" + intSlideID);
        SCORM_ClearErrorInfo();
        return SCORM_CallLMSSetValue("cmi.slide." + intSlideID + ".start");
    }
    return null;
}
...
function SCORM_GetInteractionIndex(strInteractionID){

  var intInteractionIndex;
  
  SCORM_ClearErrorInfo();
  
  WriteToDebug("in SCORM_GetInteractionIndex strInteractionID=" + strInteractionID + ", blnScorm12_allowInteractionRetake=" + blnScorm12_allowInteractionRetake);

  if (blnScorm12_allowInteractionRetake){
      // if we know the lms supports interaction retrieval and overwriting, grab the intInteractionIndex from the strID.
      intInteractionIndex = SCORM_FindInteractionIndexFromID(strInteractionID);
...
function SCORM_RecordInteraction(strID, strResponse, blnCorrect, strCorrectResponse, strDescription, 
    ...
    intInteractionIndex = SCORM_GetInteractionIndex(strID);
    ...
    if ((typeof(blnCorrect) == "boolean" && blnCorrect == true) || blnCorrect == "true" || blnCorrect == INTERACTION_RESULT_CORRECT){
        strResult = SCORM_RESULT_CORRECT;
    }

    // TODO: joe- is this going to work correctly
    // else if (blnCorrect == false || blnCorrect == "false" || blnCorrect == INTERACTION_RESULT_WRONG){  //compare against the string "false" because ("" == false) evaluates to true
    
    else if ((typeof(blnCorrect) == "boolean" && blnCorrect == false) || blnCorrect == "false" || blnCorrect == INTERACTION_RESULT_WRONG){  //compare against the string "false" because ("" == false) evaluates to true
    ...
    blnResult = blnResult && SCORM_CallLMSSetValue("cmi.interactions." + intInteractionIndex + ".type", scormInteractionType);
    ...
    if (intWeighting != undefined && intWeighting != null && intWeighting != ""){
        blnResult = blnResult && SCORM_CallLMSSetValue("cmi.interactions." + intInteractionIndex + ".weighting", intWeighting);
    }

    if (intLatency != undefined && intLatency != null && intLatency != ""){
        blnResult = blnResult && SCORM_CallLMSSetValue("cmi.interactions." + intInteractionIndex + ".latency", ConvertMilliSecondsToSCORMTime(intLatency, true));
    }
    
    if (strLearningObjectiveID != undefined && strLearningObjectiveID != null && strLearningObjectiveID != ""){
        blnResult = blnResult && SCORM_CallLMSSetValue("cmi.interactions." + intInteractionIndex + ".objectives.0.id", strLearningObjectiveID);
        WriteToDebug("Here is the cmi.interactions." + intInteractionIndex + ".objectives.0.id = "+ strLearningObjectiveID);
    }
    
    blnResult = blnResult && SCORM_CallLMSSetValue("cmi.interactions." + intInteractionIndex + ".time", ConvertDateToCMITime(dtmTime));
    

    if (blnScorm12_questionText)
    {
        blnResult = blnResult & SCORM_CallLMSSetValue("cmi.interactions." + intInteractionIndex + ".question", strDescription);
    }
...
function SCORM_RecordTrueFalseInteraction(strID, blnResponse, blnCorrect, blnCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID, dtmTime){
    ...
    WriteToDebug("In SCORM_RecordTrueFalseInteraction strID=" + strID + ", blnResponse=" + blnResponse + 
    ...
    if (blnScorm12_questionText)
    {
        strResponse = blnResponse;
        strCorrectResponse = blnCorrectResponse;
    }
    else
    {
        if (blnResponse == true){
...
function SCORM_SetObjectiveScore(strObjectiveID, intScore, intMaxScore, intMinScore){
    ...
    WriteToDebug("In SCORM_SetObjectiveScore, strObjectiveID=" + strObjectiveID + ", intScore=" + intScore + ", intMaxScore=" + intMaxScore + ", intMinScore=" + intMinScore);
...
function SCORM_GetObjectiveScore(strObjectiveID){
    ...
    WriteToDebug("In SCORM_GetObjectiveScore, strObjectiveID=" + strObjectiveID);
...
function SCORM_GetObjectiveStatus(strObjectiveID){
    ...
    WriteToDebug("In SCORM_GetObjectiveStatus, strObjectiveID=" + strObjectiveID);
...
//C.  We only want to allow this when we know the LMS supports it (i.e. check blnScorm12_allowInteractionRetake the flag)
...
function SCORM_FindInteractionIndexFromID(strInteractionID){
    ...
    if (blnScorm12_allowInteractionRetake){
        ...
        WriteToDebug("In SCORM_FindInteractionIndexFromID");
        
        intCount = SCORM_CallLMSGetValue("cmi.interactions._count");
        ...
        for (i=0; i<intCount; i++){
            ...
            strTempID = SCORM_CallLMSGetValue("cmi.interactions." + i + ".id");
