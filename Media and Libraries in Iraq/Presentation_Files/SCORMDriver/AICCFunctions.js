...
// PointeCast specific custom API
var AICC_Core_Children = "";
...
var AICC_Entry = ENTRY_REVIEW;              //must be the default b/c absense of entry flag indicates "review"
...
function AICC_SetSlideStarted(intSlideID){
    WriteToDebug("In AICC_SetSlideStarted, --ignored--");
    return null;
}
    ...
    for (i=0; i < aryAICCResponseLines.length; i++){
        ...
        if (! AICC_HasItemBeenFound(strLineName)){      //only process an item the first time it is found since only the first instance is significant
            ...
            switch (strLineName){
                ...
                case "cmi.core._children":  // SCORM 1.2 cmi.core._children
                    WriteToDebug("Item is cmi.core._children");
                    AICC_Core_Children = strLineValue;
                break;
...
function FormAICCPostData(){
    ...
    strAICCData += "Lesson_Status=" + AICC_TranslateLessonStatusToAICC() + "\r\n";
...
function AICC_TranslateLessonStatusToAICC(){
    ...
    for (var i=0; i<AICC_aryObjectivesWrite.length; i++){
        ...
        if (AICC_aryObjectivesWrite[i]){
            ...
            if (AICC_aryObjectivesWrite[i][AICC_OBJ_ARRAY_STATUS] != ""){
                strObjectives += "J_Status." + (i+1) + "=" + AICC_aryObjectivesWrite[i][AICC_OBJ_ARRAY_STATUS] + "\r\n";
            }
        }
    }
...
function DisplayAICCVariables(){
    ...
    strAlert += "AICC_Core_Children = " + AICC_Core_Children + "\n";
...
//---------------------------------------------------------------------------------
//PointeCast specific custom Get and Set Values

function AICC_GetCoreChildren(){
    WriteToDebug("In AICC_GetCoreChildren, Returning " + AICC_Core_Children);
    
    return AICC_Core_Children ; 
}
