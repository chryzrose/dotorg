...
function GetQueryStringValue(strElement, strQueryString){
	...
	if (foundValue === null){
		...
		WriteToDebug("GetQueryStringValue Element Not Found, Returning: empty string");
		return "";
	}
	else{
		WriteToDebug("GetQueryStringValue Returning: " + foundValue);
		return foundValue;
	}
}
...
function GetValueFromDataChunk(strID)
{
	...
	aryPairs = strChunk.split(DATA_CHUNK_PAIR_SEPARATOR);
	
	for (i=0;i<aryPairs.length;i++)
	{
		aryValues = aryPairs[i].split(DATA_CHUNK_VALUE_SEPARATOR);
...
function SetDataChunkValue(strID, strValue)
{
	...
	for (i=0;i<aryPairs.length;i++)
	{
		...
		if (aryValues[0]==strID) 
		{
			...
			blnFound = true;
			aryPairs[i] = aryValues[0] + DATA_CHUNK_VALUE_SEPARATOR + aryValues[1];
		}
	}
	...
	if (blnFound==true)
	{
		strChunk = aryPairs.join(DATA_CHUNK_PAIR_SEPARATOR);
	}
	else
	{
		if (strChunk=='') 
		{
			...
			strChunk = strID + DATA_CHUNK_VALUE_SEPARATOR + strValue;
		}
		else 
		{	
			//otherwise, append
			strChunk += DATA_CHUNK_PAIR_SEPARATOR + strID + DATA_CHUNK_VALUE_SEPARATOR + strValue;
		}
	}
